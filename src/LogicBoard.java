...
public class LogicBoard implements BoardParametersIE {
    ...
    public static TypeUnit turn = TypeUnit.EMPTY;
    ...
    public TypeUnit getTypeByValue(int unit){
        switch(unit){
            case 0:
                return TypeUnit.PAWN_WHITE;
            case 1:
                return TypeUnit.PAWN_BLACK;
            default:
                return TypeUnit.block;
        }
    }
    public void changeTurn() {
        LogicBoard.isBlocking = true;
        LogicBoard.turn = (LogicBoard.turn == TypeUnit.PAWN_WHITE) ? TypeUnit.PAWN_BLACK : TypeUnit.PAWN_WHITE;
    }

    public void changeTurn(Move move) {
        LogicBoard.isBlocking = (move != null && move.isFullMove() == 2);
    ...
    public  boolean isValidBlock(Point point) {
        ...
        return this.board[point.y][point.x] == 0;
    }
    ...
    public void makeMove(Move move) {
        ...
        if (isValidMove(from,to)) {
            ...
            this.playersPos[LogicBoard.turn.getUnit()] = to;
            changeTurn();
        }
    }
    public void makeBlock(Point point) {

        if (isValidBlock(point) && LogicBoard.isBlocking) {
            this.board[point.x][point.y] = -1;
            // Update player's position
           this.blockArr.add(point);
            changeTurn();
